...
//  func invadersWhoCanFire() -> [Element] {
...
//        for t in touches { self.touchDown(atPoint: t.location(in: self)) }
  ...
  func moveInvaders(){
    ...
    if(changeDirection == true){
      self.invaderSpeed *= -1 // originally set as -1
  ...
  func didBegin(_ contact: SKPhysicsContact) {
    ...
    if ((firstBody.categoryBitMask & CollisionCategories.Invader != 0) &&
      ...
      let invaderIndex = invadersWhoCanFire.index(of: theInvader)
      if(invaderIndex != nil){
    ...
    if ((firstBody.categoryBitMask & CollisionCategories.Invader != 0) &&
      ...
      let invaderIndex = invadersWhoCanFire.index(of: firstBody.node as! Invader )
      if(invaderIndex != nil){
        invadersWhoCanFire.remove(at: invaderIndex!)
      }
      theInvader.removeFromParent()
  ...
  func setupAccelerometer(){
    if motionManager.isAccelerometerAvailable {
      motionManager.startAccelerometerUpdates(to: OperationQueue.current!, withHandler: {
        (data: CMAccelerometerData?, errorOC: Error?) in
